...
// so VerifyRequest, VerifyReply and VerifyRelayError say so with
// identity.ErrPostQuantumUnavailable alone, not as a malformed frame or a
// signature that does not verify. In any other binary the same frames reach the
// signature check and are refused as invalid signatures, and nothing else. Which
// way a run goes is read from crypto/fips140, not from the check under test. CI
// runs this test both ways.
...
// the key, then the text, then the payload, then a field's range, and for a
// relay error the key, then the code's set, then the frame type.
